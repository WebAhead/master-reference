...
If you want to make a clone of the object, you can use [Object.assign()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/assign). For example:
```
var tuesdayFood = Object.assign({}, mondayFood);
```
...
Unfortunately, if there are nested objects or arrays within your object, `Object.assign()` does not clone these, but makes a reference to them. So if our 'mondayFood' looked like this: `var mondayFood = {breakfast: 'porridge', lunch: 'pizza', dinner: {food: 'paella', drink: 'orange juice'}}`, the `var tuesdayFood = Object.assign({}, mondayFood)` cloning process would not make a clone of the `{food: 'paella', drink: 'orange juice'}` inner object, and changing the drink in 'tuesdayFood.dinner.drink' would also change 'mondayFood.dinner.drink'.

What solution you'll choose for cloning in the future will depend on whatever your cloning needs are. This time, we've used an `Array.map` to clone the outer array containing the to-do tasks, and within that, we used `Object.assign()` to clone each of the objects inside. 
